...
DATA_DIR = BASE_DIR / "extracted" / "rendimento"
...
def encontrar_linha_cabecalho(file_path):
    ...
    for i in range(len(previa)):
        linha = [normalizar_texto(x) for x in previa.iloc[i].tolist()]
        if (
            "CO_ENTIDADE" in linha
            or "ID_ESCOLA" in linha
            or "CO_ESCOLA" in linha
            or "CODIGO" in linha
            or "CODIGO DA ESCOLA" in linha
        ):
            return i
    return None
...
def main():
    ...
    for file_path in arquivos:
        ...
        for _, row in df.iterrows():
            ...
            cur.execute("""
            INSERT INTO rendimento_escola (ano, co_entidade, dados_json)
            VALUES (%s, %s, %s::jsonb)
    ...
    cur.close()
    conn.close()
    print("\nImportação do rendimento finalizada.")
